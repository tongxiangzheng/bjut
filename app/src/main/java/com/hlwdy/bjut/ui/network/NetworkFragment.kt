...
fun checkLoginStatusWifi():Pair<Boolean,Boolean> {
    ...
    try {
        ...
        if (jsonString != "") {
            ...
            val res=(jsonObject.getInt("result") == 1)
            if(res){
                return Pair(true,checkLoginStatusBjut().second)
            }else{
                return Pair(false,false)
            }
        } else {
...
fun bjutNetworkLoginBjut(networkAccount: network_account_util):Pair<Boolean,Boolean>{
    ...
    val pwd=networkAccount.getUserPwd()
    val ipv6address=try{
        val ipv6Res=postUrl("http://lgn6.bjut.edu.cn/V6?https://lgn.bjut.edu.cn",setOf<String>("DDDDD=${user}","upass=${pwd}","v46s=0","v6ip=","f4serip=172.30.201.10","0MKKey="))
    ...
    val htmlData=ipv6address?.let{
        postUrl("https://lgn.bjut.edu.cn/",setOf<String>("DDDDD=${user}","upass=${pwd}","0MKKey=Login","v6ip=${it}"))
    }?:run{
        postUrl("https://lgn.bjut.edu.cn/",setOf<String>("DDDDD=${user}","upass=${pwd}","v46s=1","0MKKey="))
    }?:return Pair(false,false)
...
fun bjutNetworkLoginBjutIpv6(networkAccount: network_account_util):Boolean{
    ...
    val htmlData=postUrl("https://lgn6.bjut.edu.cn/",setOf<String>("DDDDD=${user}","upass=${pwd}","v46s=2","0MKKey="))?:return false
...
fun checkNetworkFlow():Long{
    ...
    if(flowValue==null){
        ...
        return flowValue.toLongOrNull() ?:-1
    }
}
fun formatFlowSize(sizeInBytes: Long): String {
    ...
    return String.format(Locale("zh", "CN"),"%.1f %sB", sizeInBytes / Math.pow(unit.toDouble(), exp.toDouble()), pre)
}
class NetworkFragment : Fragment() {
    ...
    private fun updateShow(){
        ...
        if(networkFlow!=-1L){
            ...
            binding.networkFlowMonitor.setText("未知")
        }
    }
    ...
    private fun loginNetwork(networkAccount: network_account_util){
        if(networkLoginipv4States==true){
            showToast("已登录")
            return
        }
    ...
    private fun loginNetworkWifiIpv6(networkAccount: network_account_util){
        if(networkLoginipv6States==true){
            showToast("已登录")
            return
        }
        if(!networkAccount.haveData()){
